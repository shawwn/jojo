...
*** dict_to_list
    ...
    dict_to_list (env_t &env, shared_ptr <dict_o> dict)
    {
        ...
        for (auto &kv: dict->obj_map) {
            ...
            auto obj = kv.second;
            auto pair = cons_c (sym, unit_list (env, obj));
...
*** word_list_head
    ...
    word_list_head (env_t &env, shared_ptr <obj_t> word_list)
    {
        ...
        if (bar_word_p (word)) {
            ...
            return cons_c (
                head, word_list_head (env, cdr (word_list)));
...
*** word_list_rest
    ...
    word_list_rest (env_t &env, shared_ptr <obj_t> word_list)
    {
        ...
        if (bar_word_p (word)) {
            ...
            return word_list_rest (env, cdr (word_list));
        else if (unquote_word_p (word))
            return word_list_rest (env, cdr (word_list));
        else
...
***** m_let
      ...
      m_let (env_t &env, obj_map_t &obj_map)
      {
          ...
          for (auto binding: binding_vect->obj_vector) {
              ...
              rest = cons_c (unit_vect (env, name), rest);
              rest = cons_c (make_sym ("lambda"), rest);
              rest = cons_c (rest, unit_list (env, obj));
          }
          env.obj_stack.push (rest);
      }
      #+end_src
    ...
    expose_bool (env_t &env)
    {
        ...
        define_prim (
            ...
            [] (env_t &env, obj_map_t &obj_map)
            {
                ...
                assert (bool_p (obj));
                env.obj_stack.push (make_bool (env, false_p (obj)));
            });
    }
    #+end_src
